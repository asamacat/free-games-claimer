...
try {
  ...
  // Bolt: Batch Playwright lookups concurrently to avoid sequential IPC overhead
  ...
  if (cfg.pg_claimdlc) {
    ...
    const dlcs = await Promise.all(cards.map(async card => {
      // Bolt: Batch Playwright lookups concurrently to avoid sequential IPC overhead
      const [game, title, urlSlug] = await Promise.all([
      ...
      return {
        game,
        title,
        url: 'https://gaming.amazon.com' + urlSlug,
      };
    }));
    // console.log(dlcs);
