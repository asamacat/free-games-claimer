...
try {
  ...
  // Bolt: Optimized element list extraction
  // using .evaluateAll instead of mapping .elementHandles() reduces IPC boundaries over N calls
  const urlSlugs = await game_loc.evaluateAll(els => els.map(a => a.getAttribute('href')));
  ...
  for (const url of urls) {
    ...
    if (await page.locator('span:text-is("About Bundle")').count()) {
      ...
      try {
        // Bolt: Optimized element text extraction
        // using .allInnerTexts() avoids creating N element handles and reduces IPC roundtrips
