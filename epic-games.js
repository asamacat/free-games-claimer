...
try {
  ...
  // ⚡ Bolt: Use `evaluateAll` to fetch all hrefs in one IPC call (O(1)) instead of fetching element handles and evaluating each one (O(N)).
  ...
  for (const url of urls) {
    ...
    if (await page.locator('span:text-is("About Bundle")').count()) {
      ...
      try {
        // ⚡ Bolt: Use `allInnerTexts` to fetch all texts in one IPC call (O(1)) instead of resolving all locators and fetching their inner text individually (O(N)).
