...
try {
  ...
  // Bolt: Replace slow Promise.all(.map()) over elementHandles with evaluateAll
  // evaluateAll executes a single time in the browser context (O(1) IPC latency) instead of crossing the boundary for each element.
  ...
  for (const url of urls) {
    ...
    if (await page.locator('span:text-is("About Bundle")').count()) {
      ...
      try {
        // Bolt: Optimized using allInnerTexts() directly instead of Promise.all mapping
        // allInnerTexts extracts all text in a single round-trip IPC call.
