...
| FGC_MASTER_KEY | | `config.env` 内のパスワードを暗号化するためのオプションのマスターキー。
...
パスワードやワンタイムパスワード（OTP）を平文で保存することはセキュリティリスクとなる可能性があるため、ご注意ください。必ず固有のパスワードまたは生成されたパスワードを使用してください。
デフォルトでは、`data/config.env` のパスワードは Base64 でエンコードされていますが、これは機密性を提供しません。
セキュリティを向上させるには、`FGC_MASTER_KEY` 環境変数を設定してください。設定されている場合、`node fgc config` は AES-256-GCM を使用してパスワードを暗号化します。
